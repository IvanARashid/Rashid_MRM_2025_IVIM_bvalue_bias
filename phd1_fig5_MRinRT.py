...
for ax_idx in range(len(grid)):
    ...
    cblabel = ["$f$ [\%]", "$D^*$ [1e-3mm$^2$/s]", "$D$ [1e-3mm$^2$/s]"] 
    if ax_idx in [0, 1, 2]:
        fontsize = 14
...
density = False
axs[0].hist(histogram_data[0]*100, density=density, bins=50, range=(-6, 6), alpha=.5)
#sns.histplot(ax=axs[0], data=histogram_data[0]*100, bins=60)
#axs[0].set_xlim(-6, 6)
axs[0].set_xticks([-6, -3, 0, 3, 6])
axs[0].set_yscale("log")
axs[0].set_xlabel("$\Delta f$ [\%]")
axs[0].set_ylabel("Difference histogram\nNominal -- Actual", fontsize=fontsize+3)

axs[1].hist(histogram_data[1]*1000, density=density, bins=100, range=(-20,20), alpha=.5)
axs[1].set_xlim(-10, 10)
#axs[1].set_xticks([-30, 0, 30])
axs[1].set_yscale("log")
axs[1].set_xlabel("$\Delta D$* [1e-3mm$^2$/s]")
